...
rec1_loan <- recipe( Current_loan_status ~ . , 
                    data = loan_train) |>
  # Eliminar manualmente la variable antes de cualquier paso
  step_rm(historical_default) |>
...
# Extrae los hiperparámetros de dicho workflow.
grid_knn_tune <- tibble(expand.grid(neighbors = c(9,10,11,12,13,14),
                                    dist_power = c(1,2)))
grid_knn_tune
```
...
rec_bag <- recipe(Current_loan_status ~ ., data = loan_train) |>
  # Eliminar explícitamente la variable conflictiva
  step_rm(historical_default) |>
...
De todas las predicciones realizadas, el 92.8% son correctas, es decir, el modelo clasifica correctamente tanto a los clientes que harán default como a los que no lo harán.

#### Sensitivity (71.3%):

Si un cliente hará default, la probabilidad de que el modelo lo detecte correctamente como tal es del 71.3%.
...
De los clientes que el modelo predice que harán default, la probabilidad de que realmente lo hagan es del 92.4%.

#### Specificity (98.5%):

Si un cliente no hará default, la probabilidad de que el modelo lo clasifique correctamente como tal es del 98.5%.
...
Si elegimos aleatoriamente a un cliente que hará default y otro que no, la probabilidad de que el modelo asigne una mayor puntuación de riesgo al cliente que hará default es del 93.5%.

------------------------------------------------------------------------

## Métricas finales

#### Detection Prevalence (16.0%):

Nos indica que el modelo asigna el 16.0% de los datos como default.
...
```{r}
# Calcular el percentil 99 de loan_amnt
p99 <- quantile(shap_num$feature_value[shap_num$variable == "loan_amnt"], 0.99, na.rm = TRUE)

# Filtrar los datos debajo del percentil 99
shap_loan_amt_filtered <- shap_num %>%
  filter(variable == "loan_amnt", feature_value < p99)

# Graficar
ggplot(shap_loan_amt_filtered, aes(x = feature_value, y = shap_value)) +
  geom_point(alpha = 0.5, size = 1.2, color = "darkblue") +
  geom_smooth(method = "loess", se = FALSE, color = "red") +
  labs(
    title = "Dependencia SHAP - Loan Amount (filtrado al percentil 99)",
    x = "Loan Amount",
    y = "SHAP Value"
  ) +
  theme_minimal(base_size = 13) +
